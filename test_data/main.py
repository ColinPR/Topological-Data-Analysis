...
def main():
    ...
    seed = 1345
    users = 100
    remove = 100 # remove some extra edges
    directed = True
    weighting = 0.5
    ...
    # Place all assets in desired locations
    distances = np.zeros((users, users))
    for source_user in range(0,users):
        ...
        model = lambda y, t : ODEModel.model(y,t,L = laplacian, source = source_user)
        ...
        #
        # plt.legend()
        distances[source_user][:] =  (t_span[1] - t_span[0]) / (assets[-1][:]-assets[0][:])
        distances[source_user][source_user] = 0
    ...
    results = ripser(distances, distance_matrix = True, maxdim = 2)['dgms']
    plot_diagrams(results, show = True)
